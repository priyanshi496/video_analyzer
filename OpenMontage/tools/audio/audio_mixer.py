...
class AudioMixer(BaseTool):
    ...
    def _duck(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        filter_complex = (
            f"[0:a]apad[speech_padded];"
            f"[1:a][speech_padded]sidechaincompress="
    ...
    def _full_mix(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        if duck_enabled and speech_tracks and music_tracks:
            ...
            if len(speech_tracks) > 1:
                filter_parts.append(
                    f"{speech_labels}amix=inputs={len(speech_tracks)}:duration=longest[speech_mix_pre]"
                )
                filter_parts.append("[speech_mix_pre]asplit[speech_sc][speech_out]")
            else:
                filter_parts.append(f"[a{speech_indices[0]}]asplit[speech_sc][speech_out]")

            filter_parts.append("[speech_sc]apad[speech_padded]")
            ...
            filter_parts.append(
                f"{music_in}[speech_padded]sidechaincompress="
                f"threshold=0.02:ratio=9:attack={attack}:release={release}:"
