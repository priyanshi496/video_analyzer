...
import { CinematicRendererProps, CinematicTone, CinematicVideoScene, CinematicScene, CinematicSoundtrack } from "./cinematic/types";
import { CaptionOverlay } from "./components/CaptionOverlay";

const FPS = 30;

function resolveScenes(props: CinematicRendererProps): CinematicScene[] {
  const scenes = props.scenes || [];
  if (scenes.length > 0) {
    return scenes;
  }
  const cuts = (props as any).cuts || [];
  if (cuts.length === 0) {
    return [];
  }
  const mapped: CinematicScene[] = [];
  ...
  for (let i = 0; i < cuts.length; i++) {
    ...
    mapped.push({
      ...
      trimBeforeSeconds: cut.in_seconds,
...
const SceneVideo: React.FC<{ scene: CinematicVideoScene }> = ({ scene }) => {
  ...
  const trimBefore =
    scene.trimBeforeSeconds !== undefined
      ...
      : undefined;
  const trimAfter =
    scene.trimAfterSeconds !== undefined && scene.trimAfterSeconds > 0
...
export const CinematicRenderer: React.FC<CinematicRendererProps> = (props) => {
  const {
    titleFontSize = 78,
    titleWidth = 1320,
    signalLineCount = 18,
  ...
  // Fallback resolution of music and soundtrack from edit_decisions.json audio props structure
  const rawAudio = (props as any).audio;
  ...
  const resolvedSoundtrack = soundtrack || (rawAudio?.narration ? {
    src: rawAudio.narration.asset_id || rawAudio.narration.src || (rawAudio.narration.segments?.[0]?.asset_id),
    volume: rawAudio.narration.volume ?? 1,
  } as CinematicSoundtrack : undefined);
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "#000000" }}>
      ...
      {resolvedSoundtrack ? (
        <Soundtrack
          src={resolvedSoundtrack.src}
          volume={resolvedSoundtrack.volume ?? 1}
          trimBeforeSeconds={resolvedSoundtrack.trimBeforeSeconds}
          trimAfterSeconds={resolvedSoundtrack.trimAfterSeconds}
          fadeInSeconds={resolvedSoundtrack.fadeInSeconds ?? 0.3}
          fadeOutSeconds={resolvedSoundtrack.fadeOutSeconds ?? 0.5}
        />
      ) : null}
      {/* Layer 2: Music bed (separate track, ducked) */}
      {resolvedMusic ? (
        <Soundtrack
          src={resolvedMusic.src}
          volume={resolvedMusic.volume ?? 0.15}
          trimBeforeSeconds={resolvedMusic.trimBeforeSeconds}
          trimAfterSeconds={resolvedMusic.trimAfterSeconds}
          fadeInSeconds={resolvedMusic.fadeInSeconds ?? 2}
          fadeOutSeconds={resolvedMusic.fadeOutSeconds ?? 3}
        />
      ) : null}
